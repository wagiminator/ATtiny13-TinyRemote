...
# Software
There are a variety of communication protocols for infrared remote controls. Basically, most of them have in common that a carrier wave between 30kHz and 58kHz, depending on the protocol, is generated by means of PWM at the IR diode so that the receiver can distinguish the signal from the noise. The message is modulated onto the carrier wave using pulse code modulation (PCM) by simply switching the IR LED on and off in a defined pattern. The messages are composed of a start frame, the device address of the receiver and the key-dependent command. The three most widely used protocols are implemented for the TinyRemote:
...
As long as a key remains down the message will be repeated every 114ms without changing the toggle bit.
...
The Sony SIRC protocol uses pulse length encoding on a carrier frequency of 40kHz. A "0" bit is a 600us burst followed by a 600us space, a "1" bit is a 1200us burst followed by a 600us space. IR message starts with a 2400us leading burst followed by a 600us space. The command and address bits are then transmitted, LSB first. Depending on the protocol version, these are in detail:
